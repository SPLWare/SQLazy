...
Partial SQLazy code: match department_number…  // the ellipsis indicates the code is incomplete and not yet runnable
...
Partial SQLazy code: match department_number; with department_table…	// This example and the previous one are two ways of expressing the same calculation.

Parameter: **join_column_name**

One or more fields in the right table used for matching. Optional parameter; omitting the parameter value means the value is the primary key; type is a field identifier or a set of field identifiers; the parameter name must be omitted.
...
Partial SQLazy code: match department_number; with department_table; department_number… This example and the two above are different expressions of the same calculation.

Parameter: **option**

The filtering method after matching, with 3 writable parameter values (enumeration values): exists, miss, not_exists. exists means keeping records that have a match and is the default parameter value; miss and not_exists are synonyms meaning keeping records that do not have a match.

Optional parameter, the default parameter value is "exists"; the default value may also be written explicitly, writing "exists" has the same effect as omitting this parameter, as in the previous example; enumerated type; the parameter name must be omitted.
...
SQLazy: match department_number; with department_table; department_number; miss

Parameter: **condition <condition_expression>**

This parameter is the condition expression for the match. It usually contains fields from the base table (left table) and the right table, where base table fields must have an @ suffix and non-base table fields must have no @ suffix, e.g., in SQLazy code: order_date>registration_date@, here order_date is a right table field and registration_date is a left table field. The role of this parameter is similar to the "where" condition in "join on… where…" in SQL, e.g., in SQL code: "select left_table.* from left_table join right_table on left_table.id=right_table.id where right_table.order_date>left_table.registration_date", the SQLazy condition expression is analogous to "right_table.order_date>left_table.registration_date" in SQL.
...
SQLazy: match department_number; with department_table; department_number; miss; condition (employee_salary@>=5000 and (["Sales Dept 1","Sales Dept 2","Product Dept"] contain department_name))
...
SQLazy: match with department_table; miss; condition (employee_salary@>=5000 and (["Sales Dept 1","Sales Dept 2","Product Dept"] contain department_name))
