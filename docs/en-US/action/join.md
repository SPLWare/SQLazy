### Action: join
Description: The right table is joined to the left table (the current/focused table) based on the specified base column, and the specified columns are attached to the left table to form a wider table. Supports left join, inner join, and full join. Note: after this action executes, the record order usually does not remain consistent with the focus table (main table); calculations related to order should be re-sorted.
...
> Partial SQLazy code: join department_number; with department_table…

Parameter: **join_column_name**
One or more fields in the right table used for the join. Required parameter; omitting the parameter value means the value is the primary key of the right table; type is a field identifier or a set of field identifiers; the parameter name must be omitted.
> Partial SQLazy code: join department_number; with department_table; department_number…
...
SQLazy: join department_number; with department_table; department_number; take department_name, office_address as address	// This is a complete code, no ellipsis.

Parameter: **option**
The join type for the association. Required parameter; if absent, the parameter value defaults to left join, as in the previous example; enumerated type; the parameter name must be omitted. There are 3 enumeration values: left, inner, full. Note: the 20260916 spec struck through the Chinese enum value 对位 but left the English word align unstruck; align is nevertheless removed from this language version and must never be output.
...
SQLazy: join department_number; with department_table; department_number; take department_manager department_address; inner; condition (employee_salary@>=5000 and (["Sales Dept 1","Sales Dept 2","Product Dept"] contain department_name))
...
SQLazy: join with department_table; take department_manager department_address; inner; condition (employee_salary@>=5000 and (["Sales Dept 1","Sales Dept 2","Product Dept"] contain department_name))
...
SQLazy: join department_number; with department_table; department_number; take department_manager department_address; inner; condition ((employee_salary@>=salary_lower and employee_salary<=salary_upper) and (["Sales Dept 1","Sales Dept 2","Product Dept"] contain department_name)) 
