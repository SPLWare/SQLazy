...
SQLazy: compute avg, ClosePrice[-2:2], as MA5
...
SQLazy: compute sum, Amount, as TotalAmountPerSeller; partition SellerID
Partial results:
...
SQLazy: compute TotalAmountPerSeller*0.1, as Reward		//The result is the same whether partitioned or not.
Note that the above two SQLazy statements can be combined into the following one (not all cases can be substituted), changing the calculation order but the result remains the same.
SQLazy: compute sum, Amount*0.1, as Reward; partition ClientID
...
SQLazy: compute (if ((OrderDate_year=2022 and OrderDate_month=3) then Amount*1.1)) , as TargetAmount
Parameter: **[with]**
...
SQLazy: compute concat, Client, with ",", as ClientList; partition SellerId
Partial results:
...
SQLazy: compute Amount[-1]*1.1, as TargetAmount; partition ClientID