...
count: understood by common sense. This enum value must also be paired with the **aggregated_expression**. If there is no explicit **aggregated_expression** in the user's instruction, use the row number field # instead (or any field name, any constant), e.g., SQLazy: summarize count #.
...
SQLazy: summarize max Amount, min Amount
Result:
maxAmount	minAmount
20000	231
...
SQLazy: summarize concat Client; with ","; as ClientList; group SellerId
Partial results:
SellerId	ClientList
1	TAS,DSGC,GC,HU
3	JFS,NR,KT,JFE,RA
...
SQLazy: summarize max Amount; condition (OrderDate_year=2019 or OrderDate_year=2020)
Result:
...
SQLazy: summarize argmax OrderDate max Amount, sum Amount
Result:
...
SQLazy: summarize max Amount, min Amount; group Year, ClientID
Result:
...
SQLazy: summarize max Amount as Large_Order_Amount, min Amount as Small_Order_Amount
Result:
