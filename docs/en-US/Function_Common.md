...
- SQLazy fragment: (OrderDate isnull)                 //0 non-object parameters
- SQLazy fragment: (1980-02-27 week_end monday_first)                    //1 unnamed non-object parameter
...
- SQLazy fragment: datetime("12/28/1972 10:23:43"; format "MM/dd/yyyy HH:mm:ss")  //2 parameters
...
F[i] F is a field name, i is the adjacent/relative/cross-row position, i.e., the i-th row relative to the current row. Positive numbers indicate forward, negative numbers indicate backward, out-of-bound returns null. F[i] represents the value of field F in the current table at the row i steps away from the current row. The current field F is an abbreviation for F[0]. For example: Assign the Amount of the Orders table to the Amount of the next record, effectively shifting one row upward. SQLazy:
