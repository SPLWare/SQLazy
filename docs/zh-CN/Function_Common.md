...
Note: This prompt focuses only on expressions and functions. The definition of Action is provided to help you distinguish between Action and expression (expression is part of an Action) and points of confusion (Actions and functions may occasionally be similar in description, but they are fundamentally different in result type). This prompt only processes function/expression parts; an Action is definitely not a function, so do not process Action/non-expression parts.
