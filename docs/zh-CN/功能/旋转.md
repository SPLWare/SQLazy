...
参数：**计算方法**
有7种区别较大的计算方法：首个、合计、计数、平均、最大、最小、逆向。非必要参数，缺省参数值是首个；枚举类型；参数名必须省略。
合计、计数、平均、最大、最小、首个：这是一组聚合算法，即行转列旋转后，标题列的一行对应取值列的多个值时，可以通过聚合算出单一值。这种情况一般是因为分组列、标题列、取值列之外还有原表没用到的列。
...
branch3				12000	24000
SQLazy: 旋转 分组 Branch, Dept; 标题列 State; 取值列 Amount量; 最大
...
branch3				11000	10000
SQLazy: 旋转 分组 Branch, Dept; 标题列 State; 取值列 Amount量
解释：首个是计算方法的默认参数值，上面代码省去了。
> 对前面的分公司部门预算表，按Branch分组，将组内记录行转列，State是标题列，Amount量是取值列，对Amount量计数。
SQLazy: 旋转 分组 Branch, Dept; 标题列 State; 取值列 Amount量; 计数
解释：每格计数为该Branch下该State对应的Dept分组记录条数（与合计同属聚合分支），便于与合计对比例证。
...
SQLazy: 旋转 分组 Branch, Dept; 标题列 State; 取值列 Amount量; 表头 FState,Cstate,NState,TState,PState
结果：
